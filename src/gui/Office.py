...
from utils.game_state import office_state
...
class Office:
    def __init__(self, screen_dimension: tuple):
        ...
        self.camera_button = OfficeButton(
            pygame.Color("red"),
            self.__width / 4,
            self.__height * 9 / 10,
            self.__width / 2,
        ...
        self.window_button = OfficeButton(
            pygame.Color("red"),
            self.__width * 9 / 10,
            self.__height / 4,
            self.__width / 30,
        ...
        self.office_front_background = image(
            get_resource_path("/assets/images/office_front_lights.jpeg"),
            screen_dimension,
            1,
        )

        self.office_back_background = image(
            get_resource_path("/assets/images/office_back_lights.jpg"),
            screen_dimension,
            1,
        )

        self.office_front_open_background = image(
            get_resource_path("/assets/images/office_front_lights_open.jpg"),
            screen_dimension,
            1,
        )

        self.office_back_open_background = image(
    ...
    def render_office(self, screen, office_state):
        ...
        if (
            office_state is office_state.OFFICE_FRONT
            or office_state is office_state.OFFICE_FRONT_OPEN
        ):
        ...
        if (
            office_state is office_state.OFFICE_BACK
            or office_state is office_state.OFFICE_BACK_OPEN
        ):
            if self.camera_x == self.office.get_width() - self.__width:
                self.window_button.render_button(screen)
            if self.camera_x == 0:
                self.front_office_button.render_button(screen)
